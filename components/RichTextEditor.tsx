...
type RichTextEditorProps = {
    ...
    placeholder?: string;
    /** Smaller toolbar/padding for inline contexts like comments. Defaults to false (full size, for post/project bodies). */
    compact?: boolean;
};
...
/** Shared TipTap rich text editor. Not wired into any form/comment yet — that's a follow-up task. */
const RichTextEditor = ({ value, onChange, placeholder = "Write something...", compact = false }: RichTextEditorProps) => {
    ...
    return (
        <div className="rich-text-editor border-dark-200 flex flex-col overflow-hidden rounded-[6px] border">
            <div
                className={`border-dark-200 bg-dark-200/40 relative flex flex-row flex-wrap items-center gap-1 border-b ${
            ...
            >
                ...
                {showEmojiPicker && (
                    <div className="border-dark-200 bg-dark-100 absolute top-full left-0 z-20 mt-1 grid grid-cols-6 gap-1 rounded-[6px] border p-2 shadow-lg">
                        {EMOJI.map((emoji) => (
                            <button
                                key={emoji}
                                type="button"
